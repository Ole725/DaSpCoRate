# /DaSpCoRate/backend/api/app/crud/rating.py
...
def get_ratings_by_couple(db: Session, couple_id: int, skip: int = 0, limit: int = 100) -> List[models_rating.Rating]:
...
def get_ratings_by_session(db: Session, session_id: int, skip: int = 0, limit: int = 100) -> List[models_rating.Rating]:
    return db.query(models_rating.Rating).filter(
...
def get_ratings_for_trainer_sessions(db: Session, trainer_id: int, skip: int = 0, limit: int = 100) -> List[models_rating.Rating]:
...
def create_rating(db: Session, rating_in: schemas_rating.RatingCreate):
...
def update_rating(db: Session, rating_id: int, rating_in: schemas_rating.RatingUpdate):
