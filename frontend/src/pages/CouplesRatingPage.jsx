...
function CouplesRatingPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const [ratingsData, sessionsData, coupleData] = await Promise.all([
          getRatingsByCoupleId(coupleId),
          getSessions(), 
    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
        <h2 className="text-3xl font-bold text-gray-800 dark:text-gray-100">
          {coupleInfo ? `Wertungen für ${coupleInfo.mrs_first_name} & ${coupleInfo.mr_first_name}` : 'Wertungen'}
        </h2>
      ...
      {sessionsWithRatings.length === 0 && (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg">
          <p>Für dieses Paar wurden noch keine Bewertungen erfasst.</p>
        </div>
      )}
...
export default CouplesRatingPage;