          ...
          <tbody>
            {sortedCouples.map(couple => (
              <tr key={couple.id} className="hover:bg-gray-50">
                <td className="sticky left-0 bg-white border p-2 font-semibold z-10 min-w-[150px] group relative">
                    ...
                    <td key={criterion.key} className="border p-2 text-center">
                      <div className="flex justify-center gap-2">
                        {[1, 2, 3].map(points => {
                          ...
                          const buttonClasses = `w-8 h-8 rounded-full font-bold flex items-center justify-center transition-colors ${isSelected ? 'bg-green-500 text-white' : 'bg-gray-200 hover:bg-gray-300'}`;
